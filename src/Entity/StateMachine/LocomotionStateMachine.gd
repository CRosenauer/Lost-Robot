...
func CanJump(inputArr, edgeArr):
	return STATES.LocomotionStates.Grounded && (inputArr[INPUTS.Input_Jump] == 1)

func CanDoubleJump(inputArr, edgeArr):
	return STATES.LocomotionStates.Jump && (edgeArr[INPUTS.Input_Jump] == 1) && !m_usedDoubleJump

func CanAirDash(inputArr, edgeArr):
	return STATES.LocomotionStates.Grounded && (edgeArr[INPUTS.Input_Dash] == -1)
...
func CanWallJump(inputArr, edgeArr):
	return STATES.LocomotionStates.PreWallJump && IsPressingOppositeDirection(inputArr) && edgeArr[INPUTS.Input_Jump]

func CanRun(inputArr, edgeArr):
	return STATES.LocomotionStates.Grounded && (inputArr[INPUTS.Input_Dash] == 1)
